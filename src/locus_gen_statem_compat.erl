...
enter_loop(Module, Opts, State, Data, Server, Actions) ->
    ...
    Parent = locus_gen_compat:get_parent(),
...
enter(Module, Opts, State, Data, Server, Actions, Parent) ->
    ...
    Name = locus_gen_compat:get_proc_name(Server),
    Debug = locus_gen_compat:debug_options(Name, Opts),
    Events = [],
    P = [],
...
init_it(Starter, Parent, ServerRef, Module, Args, Opts) ->
    ...
    Class:Reason ->
        ...
        Name = locus_gen_compat:get_proc_name(ServerRef),
        locus_gen_compat:unregister_name(ServerRef),
...
-spec wakeup_from_hibernate(_,_,#{}) -> any().
...
-spec loop_hibernate(pid(), term(), #{}) -> no_return().
loop_hibernate(Parent, Debug, S) ->
